...
def drawCube(img, ar_list, ar_id, camera_matrix, dist_coeff):
        ...
        m = markerLength/2
        ######################## INSERT CODE HERE ########################
        pts = np.float32([[0,0,0],[0,m,0],[m,m,0],[m,0,0],[0,0,-m],[0,m,-m],[m,m,-m],[m,0,-m]])
        pt_dict = {}
        imgpts, _ = cv2.projectPoints(pts, rvec, tvec, camera_matrix, dist_coeff)
        for i in range(len(pts)):
                 pt_dict[tuple(pts[i])] = tuple(imgpts[i].ravel())
        ...
        img = cv2.line(img, src, dst5, (0,0,255), 4)
        img = cv2.line(img, src, dst6, (0,0,255), 4)
        img = cv2.line(img, src, dst7, (0,0,255), 4)
        return img
...
if __name__=="__main__":
        ...
        img = cv2.imread("..\\TestCases\\image_6.jpg")
